...
echo "Target Date: "${TGDATE}

LOGFILE=${GPKRUTIL}/cronlog/cron_log_load_${TGDATE}.log
...
EXECUTE E'cat /data/master/gpseg-1/pg_log/gpdb-${TGDATE}*.csv' ON MASTER
FORMAT 'csv' (delimiter as ',' quote as '"')
ENCODING 'utf8'
SEGMENT REJECT LIMIT 100000;

delete from dba.sql_history
where  event_time >= '${TGDATE}'::timestamp
and    event_time <  '${TGDATE}'::timestamp + '1 day'::interval ;
